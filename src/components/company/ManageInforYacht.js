...
const ManageInforYacht = (props) => {
  ...
  const handleUpdateYacht = async () => {
    ...
    let res = await updateYacht(
      idYacht,
      dataUpdate.name.trim(),
      image,
      dataUpdate.hullBody.trim(),
      dataUpdate.description.trim(),
      dataUpdate.rule.trim(),
      dataUpdate.itinerary.trim(),
      idYachtType,
      idLocation
    );
            ...
            <Form>
              <Row className="mb-3">
                <Form.Group as={Col}>
                  ...
                  <Form.Control name="name" type="text" onChange={handleChange} value={dataUpdate.name} />
                </Form.Group>

                <Form.Group as={Col}>
                  <Form.Label>Hull-Body</Form.Label>
                  <Form.Control name="hullBody" type="text" onChange={handleChange} value={dataUpdate.hullBody} />
                </Form.Group>
              </Row>
              <Row className="mb-3">
                <Form.Group as={Col}>
                  <Form.Label>Itinerary</Form.Label>
                  <Form.Control name="itinerary" type="text" onChange={handleChange} value={dataUpdate.itinerary} />
                </Form.Group>
              </Row>
              <Row className="mb-3">
                <Form.Group as={Col}>
                  <Form.Label>Rule</Form.Label>
                  <Form.Control name="rule" type="text" onChange={handleChange} value={dataUpdate.rule} />
                </Form.Group>
                <Form.Group as={Col}>
              ...
              <div className="col-md-12 img-preview">
                {previewImage ? <img src={previewImage} alt="image upload" /> : <span>Update Avartar</span>}
              </div>

              <Button onClick={handleUpdateYacht} className="my-3 text-center">
                Update Now
              </Button>
            </Form>
          </Accordion.Body>
