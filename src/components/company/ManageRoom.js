...
const ManageRoom = () => {
  ...
  const filterAndPaginateRoom = () => {
    const filtered = listRoom.filter((y) => y.name.toLowerCase().includes(searchNameRoom.toLowerCase().trim()));
  ...
  return (
    <div className="container">
      ...
      <hr />
      <div className="row">
        <Button className="col-2 btn btn-success" onClick={() => setIsShowModalCreateRoom(true)}>
          <FaCirclePlus style={{ marginRight: 8, marginBottom: 5 }} />
          Add New Room
        </Button>

        <Button className="col btn btn-warning mx-2" onClick={() => setIsShowModalRoomType(true)}>
          <FaCirclePlus style={{ marginRight: 8, marginBottom: 5 }} />
          Manage Room Type
        </Button>
      ...
      {displayedRoom &&
        displayedRoom.length > 0 &&
        displayedRoom.map((room) => (
          <div key={room.idRoom} className="d-flex my-5 room p-3 row " style={{ gap: 50 }}>
            ...
            <div className=" col-md-4">
              ...
              <div>
                {room.area} <TbMeterSquare size={25} className="pb-1" />
              </div>
            ...
            <div className="col-md-4">
              <div className="ok">
                <Button onClick={() => handlManageImageRoom(room.idRoom)} className="btn btn-warning mx-3">
                  Manage Room Image{" "}
                </Button>
                <Button onClick={() => handleUpdateRoom(room)} className="btn btn-primary">
                  Update
                </Button>
              </div>
            </div>
